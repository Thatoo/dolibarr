...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_azurtitre.modules.php
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele Cyan avec des lignes de titre
				si <h et qty = 0 alors titre => sous total ; si <div> et qty = 0 alors comment
 */
         ...
         if ($conf->propal->multidir_output[$conf->entity]) {
             ...
             if (file_exists($dir)) {
                 ...
                 for ($i = 0; $i < $nblines; $i++) {
                     ...
                     // VAT Rate
                     if ($this->getColumnStatus('vat') && empty($isTitre) && empty($isComment)) {
                     ...
                     if ($this->getColumnStatus('subprice') && empty($isTitre) && empty($isComment)) {
                     ...
                     // Quantity
                     // Enough for 6 chars
                     if ($this->getColumnStatus('qty') empty($isTitre) && empty($isComment)) {
                     ...
                     // Unit
                     if ($this->getColumnStatus('unit') empty($isTitre) && empty($isComment)) {
                     ...
                     // Discount on line
                     if ($this->getColumnStatus('discount') && $object->lines[$i]->remise_percent && empty($isTitre) && empty($isComment)) {
                     ...
                     if ($this->getColumnStatus('totalexcltax') && empty($isTitre) && empty($isComment)) {
                     ...
                     // Add line
                     if (!empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1) && !empty($isTitre)) {
                         $pdf->setPage($pageposafter);
